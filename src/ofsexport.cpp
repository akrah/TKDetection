...
namespace OfsExport {
  ...
  namespace {
	...
	void displayExportedVertex( const Billon &billon, const Marrow &marrow, QVector<rCoord2D> vectVertex,
								const Interval<int> &interval,const int &resolutionCercle, QTextStream &stream,
								bool normalized,  bool displayBegEndFaces  ){
	  const int width = billon.n_cols;
	  const int height = billon.n_rows;
	  const int firstMarrow = interval.min() - marrow.interval().min();
	  const int nbSlices=interval.size();
	  const int lastMarrow = qMin(firstMarrow + nbSlices,marrow.size());
	  qreal depth = interval.min();
	  stream << endl;
	...
	for(int i=0; i< resolutionCercle; i++){
	  if((k==firstMarrow) && i==0 && displayBegEndFaces)
		stream << (normalized? 0:(width/2.0)) << ' ' << (normalized? 0:(height/2.0)) << ' ' << -0.5 << endl;
	  else if((k==lastMarrow) && i==0 && displayBegEndFaces)
