...
Lowess::Lowess( const qreal &bandwidth, const uint &robustness  ) : _bandWidth(bandwidth), _robustness(robustness)
{
}
...
const uint &Lowess::robustness() const
{
	return _robustness;
}

void Lowess::setRobustness( const uint &robustness )
{
...
void Lowess::compute2( const QVector<qreal> &datas, QVector<qreal> &interpolatedDatas, QVector<qreal> &residus ) const
{
	// Create test dataset.
	const int &size = datas.size();
	interpolatedDatas.resize(size);
