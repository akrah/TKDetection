...
namespace V3DExport {
	...
	namespace {
				...
				stream.writeStartElement("component");
					...
					stream.writeStartElement("coord");
						...
						stream.writeStartElement("z");
							stream.writeCharacters(QString::number(interval.minValue()));
						stream.writeEndElement();
					stream.writeEndElement();
					...
					stream.writeStartElement("coord");
						...
						stream.writeEndElement();
						stream.writeStartElement("z");
							stream.writeCharacters(QString::number(interval.maxValue()));
						stream.writeEndElement();
					stream.writeEndElement();
					...
					stream.writeStartElement("binarydata");
						...
						for ( int k=interval.minValue(); k<=interval.maxValue(); k++ ) {
		...
		void appendMarrow( const Marrow &marrow, QXmlStreamWriter &stream ) {
			...
			for ( int k=0, sliceIdx = marrow.interval().minValue() ; k<marrow.size() ; ++k, ++sliceIdx ) {
